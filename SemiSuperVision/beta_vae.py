...
class BetaVAE(BaseVAE):
    ...
    def loss_function(
        ...
        z:       torch.Tensor,   # target label for semi-supervision in [0, 1]
        is_labeled: torch.Tensor,# bool for label
        mu:      torch.Tensor,   # B×d
        ...
        if mask.sum() > 0:
            ...
            semisuper_loss = BCE_loss(mu_labeled, z_labeled)

        else:
            semisuper_loss = torch.tensor(0., device=x.device)
        
        total = recon_loss + self.beta * kld + semisuper_loss
        return total, recon_loss, kld, semisuper_loss
