...
def basic_metrics(vae: BetaVAE, loader: DataLoader, device: torch.device) -> None:
    ...
    free_nats = float(getattr(vae.config.criterion, "free_nats", 0.0))
    budgeted = torch.clamp(kld - free_nats * vae.latent_dim, min=0.0)
    print(f"True KL  (mean):    {kld.mean():.6f}")
    print(f"Budgeted KL (mean): {budgeted.mean():.6f}\n")


def mu_variance(vae: BetaVAE, loader: DataLoader, device: torch.device) -> np.ndarray:
    mus: list[Tensor] = []
    with torch.no_grad():
        for xb, _ in loader:
            xb = xb.to(device)
...
def plot_recon(vae: BetaVAE, loader: DataLoader, device: torch.device, n: int = 6) -> None:
    imgs, _ = next(iter(loader))
    imgs = imgs.to(device)
    with torch.no_grad():
        logs = vae(imgs)
        recon = logs.img.detach().cpu()
    imgs = imgs.cpu()
...
def compute_latent_stats(mu: np.ndarray, logvar: np.ndarray) -> LatentStats:
    var = np.exp(logvar)
    kl = 0.5 * (mu ** 2 + var - logvar - 1.0)
    return LatentStats(mu_mean=mu.mean(axis=0), mu_var=mu.var(axis=0), kl_mean=kl.mean(axis=0))
